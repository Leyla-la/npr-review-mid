...
public class Server {
    ...
    // Simple user store (load from users.properties if present)
    ...
    public static void main(String[] args) throws IOException {
        // Set SSL system properties following the LoginServer example in secure.txt
        // Update paths/passwords to match your keystore/truststore files.
        try {
        ...
        // Load users (users.properties) or fallback to a demo account
        loadUsers();

        // Create a small GUI for server logs (non-blocking)
        SwingUtilities.invokeLater(() -> {
            serverFrame = new JFrame("SUPER SERVER");
            serverFrame.setSize(800, 520);
            serverFrame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
            serverFrame.setLayout(new BorderLayout());

            // Top toolbar with user editor button
            JPanel topBar = new JPanel(new FlowLayout(FlowLayout.LEFT));
            JButton editUsersBtn = new JButton("Edit Users");
            editUsersBtn.addActionListener(e -> SwingUtilities.invokeLater(Server::openUserEditor));
            topBar.add(editUsersBtn);
            serverFrame.add(topBar, BorderLayout.NORTH);

            serverLogArea = new JTextArea();
            serverLogArea.setEditable(false);
            serverLogArea.setFont(new Font("Monospaced", Font.PLAIN, 12));
            serverFrame.add(new JScrollPane(serverLogArea), BorderLayout.CENTER);
        ...
        // Create SSLServerSocket using JSSE default factory (system properties above)
        try {
            SSLServerSocketFactory ssf = (SSLServerSocketFactory) SSLServerSocketFactory.getDefault();
            try (SSLServerSocket serverSocket = (SSLServerSocket) ssf.createServerSocket(PORT)) {
                String startLine = "=== SUPER SERVER RUNNING (TLS) ON PORT " + PORT + " ===";
    ...
    // Load users from users.properties file if exists; otherwise populate a demo user
            ...
            try (FileInputStream fis = new FileInputStream(f)) {
                ...
                for (String k : p.stringPropertyNames()) {
                    ...
                    if (v.startsWith("sha256$")) {
                        users.put(k, v.substring(7));
                    } else if (v.matches("[0-9a-fA-F]{64}")) {
                        users.put(k, v);
                    } else {
                        // plain text in file - hash it in memory
                        try { users.put(k, hashPassword(v)); } catch (Exception ex) { users.put(k, v); }
                    }
                }
                guiLog("Loaded users from users.properties (hashed in memory)");
                // Persist hashed form back to file for improved security
                saveUsersToFile();
                return;
            } catch (IOException ignored) {}
        }
        // fallback demo user: change this password or create users.properties for real use
        try {
            users.put("20230001", hashPassword("123456"));
        } catch (Exception e) { users.put("20230001", "123456"); }
        guiLog("Using fallback user store (one demo user 20230001 / password 123456)");
    }
    ...
    private static boolean verifyPassword(String username, String password) {
        try {
            ...
            return MessageDigest.isEqual(hashed.getBytes(StandardCharsets.UTF_8), stored.getBytes(StandardCharsets.UTF_8));
        } catch (Exception e) { return false; }
    }

    /*
     * BCrypt example (commented):
     * The code below demonstrates how you would use BCrypt instead of SHA-256.
     * This block is intentionally commented out so it does not change runtime behavior.
     * To enable BCrypt, add the jBCrypt library to your project (Maven: org.mindrot:jbcrypt:0.4)
     * or download the jar and add it to the classpath.
     */
    // import org.mindrot.jbcrypt.BCrypt; // example import (commented)
    // // BCrypt hashing: returns the bcrypt hashed string (includes salt)
    // private static String bcryptHash(String password) {
    //     // gensalt(12) uses cost factor 12 — adjust according to CPU budget
    //     return BCrypt.hashpw(password, BCrypt.gensalt(12));
    // }
    //
    // // BCrypt verification
    // private static boolean bcryptVerify(String plainPassword, String storedHash) {
    //     // storedHash is the full bcrypt hash string
    //     return BCrypt.checkpw(plainPassword, storedHash);
    // }
    //
    // // Example: when persisting, you might mark bcrypt hashes with a prefix
    // // users.properties -> user=bcrypt$<bcryptHash>
    //
    // // Helper to recognize bcrypt entries
    // private static boolean isBcryptHash(String s) {
    //     return s != null && s.startsWith("bcrypt$");
    // }
    //
    // // Example replacement verify when using both schemes:
    // private static boolean verifyPasswordWithOptionalBcrypt(String username, String password) {
    //     String stored = users.get(username);
    //     if (stored == null) return false;
    //     try {
    //         if (isBcryptHash(stored)) {
    //             String real = stored.substring("bcrypt$".length());
    //             return BCrypt.checkpw(password, real);
    //         } else {
    //             // fallback to SHA-256 hex
    //             String hashed = hashPassword(password);
    //             return MessageDigest.isEqual(hashed.getBytes(StandardCharsets.UTF_8), stored.getBytes(StandardCharsets.UTF_8));
    //         }
    //     } catch (Exception e) { return false; }
    ...
    // Persist users map to users.properties with sha256$ prefix
    private static synchronized void saveUsersToFile() {
        Properties p = new Properties();
        for (Map.Entry<String,String> e : users.entrySet()) {
            p.setProperty(e.getKey(), "sha256$" + e.getValue());
        }
        try (FileOutputStream fos = new FileOutputStream("users.properties")) {
            p.store(fos, "User store (passwords hashed with SHA-256)");
            guiLog("Saved users.properties (hashed)");
        } catch (IOException ex) { guiLog("Failed to save users.properties: " + ex.getMessage()); }
    }

    // Open a simple user editor dialog allowing add/remove and saving
    private static void openUserEditor() {
        JDialog dlg = new JDialog(serverFrame, "User Editor", true);
        dlg.setSize(420, 360);
        dlg.setLayout(new BorderLayout());
        DefaultListModel<String> model = new DefaultListModel<>();
        users.keySet().stream().sorted().forEach(model::addElement);
        ...
        add.addActionListener(e -> {
            String id = JOptionPane.showInputDialog(dlg, "Enter user id (student id):");
            if (id == null || id.trim().isEmpty()) return;
            String pwd = JOptionPane.showInputDialog(dlg, "Enter password for " + id + ":");
            if (pwd == null) return;
            try { users.put(id.trim(), hashPassword(pwd)); model.addElement(id.trim()); } catch (Exception ex) { JOptionPane.showMessageDialog(dlg, "Failed to hash password: " + ex.getMessage()); }
        });
        remove.addActionListener(e -> {
            String sel = list.getSelectedValue(); if (sel == null) return;
            users.remove(sel); model.removeElement(sel);
        });
        save.addActionListener(e -> { saveUsersToFile(); JOptionPane.showMessageDialog(dlg, "Saved users.properties"); });
        close.addActionListener(e -> dlg.dispose());
        south.add(add); south.add(remove); south.add(save); south.add(close);
    ...
    private static class ClientHandler implements Runnable {
        ...
        public void run() {
            try {
                // Authentication: expect username then password (both UTF)
                String user = dataIn.readUTF();
                String pass = dataIn.readUTF();
                if (user == null || pass == null) { send("AUTH_FAIL"); return; }
                if (!verifyPassword(user, pass)) {
                    send("AUTH_FAIL");
                    guiLog("Auth failed for user " + user + " from " + socket.getInetAddress());
                    socket.close();
                    return;
                }
                studentID = user; send("AUTH_OK");

                // CASE 1: now send ID^4 and join main
